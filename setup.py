...
DATA_FILES = [
    ...
    ('/usr/share/touchpad-indicator',
     glob.glob('src/*')),
    ('/usr/share/touchpad-indicator',
     ['data/touchpad-indicator-autostart.desktop']),
    ('/usr/share/touchpad-indicator/icons',
     glob.glob('data/icons/*.svg')),
    ('/usr/share/touchpad-indicator/social',
     glob.glob('data/social/*.svg')),
    ('/usr/share/touchpad-indicator/icons',
     ['data/icons/touchpad-indicator.svg']),
    ('/usr/share/glib-2.0/schemas',
     ['data/schemas/org.gnome.settings-daemon.plugins.media-keys.\
custom-keybindings.touchpad-indicator.gschema.xml']),
    ('/etc/pm/sleep.d',
     ['data/00_check_touchpad_status']),
    ('/usr/lib/systemd/system-sleep/',
     ['data/00_check_touchpad_status_systemd']),
    ('/usr/share/applications',
     ['data/extras-touchpad-indicator.desktop'])]
...
def babilon():
    ...
    print('############################################################')
    files_file = list_src()
    command = 'xgettext --msgid-bugs-address=%s --language=Python --keyword=_ \
--keyword=N_ --output=%s --files-from=%s' % (AUTHOR_EMAIL,
                                             TEMPLATE,
                                             files_file)
    print(ejecuta(command))
        ...
        for path, names, filenames in os.walk(po_dir):
            for f in filenames:
                if f.endswith('.po'):
                    ...
                    dest_path = os.path.join('build', 'locale-langpack', lang,
                                             'LC_MESSAGES')
